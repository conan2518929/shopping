...
import com.car.shopping.activity.HDShopDetailActivity;
import com.car.shopping.entity.CommentsInfo;
...
public class CommentDetailAdapter extends BaseAdapter {
	...
	private CommentDetailClickListener listener;
	private int number;
	private ListView mListView;
	
	public CommentDetailAdapter(Context mContext, List<CommentsInfo> list, int number) {
		this.list = list;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (list.size() > 0) {
			...
			if (info.getFather_id().equals("0")) {
				...
				spanableInfo.setSpan(new ForegroundColorSpan(mContext.getResources().getColor(R.color.blue_one)), 0, info.getUser_name().length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

				spanableInfo.setSpan(new Clickable(position, 1, info, number), 0, info.getUser_name().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
				spanableInfo.setSpan(new Clickable(position, 0, info, number), info.getUser_name().length() + 2, info.getUser_name().length() + 2 + info.getContent().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
				vp.content.setText(spanableInfo);

				vp.content.setMovementMethod(LinkMovementMethod.getInstance());
			} else {
				SpannableString spanableInfo = new SpannableString(info.getUser_name() + "回复" + info.getFather_user_name() + ":" + info.getContent());

				spanableInfo.setSpan(new ForegroundColorSpan(mContext.getResources().getColor(R.color.blue_one)), 0, info.getUser_name().length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

				spanableInfo.setSpan(new ForegroundColorSpan(mContext.getResources().getColor(R.color.blue_one)), info.getUser_name().length() + 2, info.getUser_name().length() + 2 + info.getUser_name().length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

				spanableInfo.setSpan(new Clickable(position, 2, info, number), 0, info.getUser_name().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);// 点击的是第1个名字

				spanableInfo.setSpan(new Clickable(position, 3, info, number), // 点击的是第2个名字
						info.getUser_name().length() + 2, info.getUser_name().length() + 2 + info.getFather_user_name().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

				spanableInfo.setSpan(new Clickable(position, 4, info, number), // 点击的是内容
						info.getUser_name().length() + 2 + info.getFather_user_name().length() + 1, info.getUser_name().length() + 2 + info.getFather_user_name().length() + 1 + info.getContent().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	public static abstract class CommentDetailClickListener implements OnClickListener {
	...
	class Clickable extends ClickableSpan {
		...
		private CommentDetailClickListener mListenerTwo = new CommentDetailClickListener() {
			@Override
