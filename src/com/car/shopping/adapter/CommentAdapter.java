	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (list.size() > 0) {
			...
			if (info.getFather_id().equals("0")) {
				...
				spanableInfo.setSpan(new ForegroundColorSpan(mContext.getResources().getColor(R.color.blue_one)), info.getUser_name().length() + 2,
						info.getUser_name().length() + 2 + info.getUser_name().length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				...
				spanableInfo.setSpan(
						new Clickable(position, 3, info, number), // 点击的是第2个名字
						info.getUser_name().length() + 2, info.getUser_name().length() + 2 + info.getFather_user_name().length(),
						Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

				spanableInfo.setSpan(new Clickable(position, 4, info, number), // 点击的是内容
						info.getUser_name().length() + 2 + info.getFather_user_name().length() + 1, info.getUser_name().length() + 2
								+ info.getFather_user_name().length() + 1 + info.getContent().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
