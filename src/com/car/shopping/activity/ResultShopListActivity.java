...
public class ResultShopListActivity extends BaseActivity {
...
//									System.out.println("商家==" + lists_copy.get(m).getShop_name());
					...
					Utils.showText(ResultShopListActivity.this, "请您登录后进行聊天...");
				}
...
//								System.out.println("商家==" + lists_copy.get(m).getShop_name());
						...
						AlertDialog.Builder builder2 = new Builder(ResultShopListActivity.this);
		...
		ll.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (lists.size() > 0) {
					if (!isAll) {
						Utils.changeColor(ResultShopListActivity.this, add_all, R.drawable.white_y_g);
						isAll = true;
						...
						Utils.changeColor(ResultShopListActivity.this, add_all, R.drawable.white_y);
						isAll = false;
						// 遍历list的长度，将MyAdapter中的map值全部设为true
						for (int i = 0; i < lists.size(); i++) {
	...
	private void updateAlias(String name, int position) {
		Intent intent = new Intent(ResultShopListActivity.this, ChatActivity.class);
	...
	private void getDetail(int position) {
		...
		Utils.goOtherWithDataActivity(ResultShopListActivity.this, ShopDetailActivity.class, data);
	}

	@SuppressLint({ "InflateParams", "RtlHardcoded" })
	@SuppressWarnings("deprecation")
	private void showDialog(final int position, View v) {
		View view = LayoutInflater.from(ResultShopListActivity.this).inflate(R.layout.tel_number_txl, null);
		...
		dialog = new Dialog(ResultShopListActivity.this, R.style.ActionSheetDialogStyle);
	...
	private void getCallShop(final int position) {
		...
		StringRequest stringRequest = new StringRequest(Request.Method.POST, Constant.URL_TEST + Urls.CALL_SHOP, new Response.Listener<String>() {
			...
			public void onErrorResponse(VolleyError volleyError) {
				dismissLoadingDialog();
				Utils.showText(ResultShopListActivity.this, "网络访问失败");
			}
		}) {
	...
	public void getResultData(final String  order_by) {
		AppContext.getInstance().cancelPendingRequests(TAG);
		showLoadingDialog();
		StringRequest stringRequest = new StringRequest(Request.Method.POST, Constant.URL_TEST + Urls.SEARCH_SHOP, new Response.Listener<String>() {

			@Override
			public void onResponse(String result) {
				try {
					JSONObject json = new JSONObject(result);
					System.out.println("2获取搜索结果==" + json.toString());
			...
			protected Map<String, String> getParams() throws AuthFailureError {
				...
				if(typeNum == 1){
					params.put("search_field", "am_brand,am_series");
				}else if(typeNum == 2){
					params.put("search_field", "am_series,am_brand");
				}else if(typeNum == 7){//单项产品
//					params.put("search_field", "am_part");
				}else if(typeNum == 8){
//					params.put("search_field", "am_accessory");
				}
				System.out.println("222city_id="+cityId+",search_content="+name+",order_by="+order_by+",typeNum="+typeNum);
				return params;
			}
		};
		AppContext.getInstance().addToRequestQueue(stringRequest, TAG);
		...
		if (status == 200) {
			...
			if (baseInfo != null) {
				...
				if (lists != null) {
					num = lists.size();
					if (num > 0) {
						adapter = new ResultShopListAdapter(ResultShopListActivity.this, lists, mListener);
						lv.setAdapter(adapter);
					} else {
						Utils.showText(ResultShopListActivity.this, "暂无搜索结果...");
					}
				}
			}
		}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == CAMERA_REQUEST_CODE) {
			...
			pathStr = getRealFilePath(ResultShopListActivity.this, uri);
			...
			pathStr = getRealFilePath(ResultShopListActivity.this, uriLocal);
			showLoadingDialog();
...
//				}
	...
	private void dealTalkData(String result, int type) {
		...
		if (status == 200) {
			...
			Utils.showText(ResultShopListActivity.this, "发送信息成功...");
		} else if (status == 300) {
			logout();
		} else {
			Utils.showText(ResultShopListActivity.this, "发送信息失败...");
		}
	}
	...
	private void dealLogoutShop(String result) {
		...
		if (status == 200) {
			...
			sendBroadcast(intent);
			Intent i = new Intent(ResultShopListActivity.this, LoginActivity.class);
			startActivity(i);
		}
	}
